...
class KeyboardMonitor {
    ...
    private func callKasaHelper(action: String) {
        // Get path to kasa_helper.py
        let executablePath = Bundle.main.executablePath ?? ""
        let projectDir = URL(fileURLWithPath: executablePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let helperPath = projectDir.appendingPathComponent("mydesklight_core/kasa_helper.py").path
        
        // Call helper script
        let task = Process()
        task.executableURL = URL(fileURLWithPath: "/usr/bin/env")
        task.arguments = ["python3", helperPath, action]
        
        do {
            try task.run()
        } catch {
            // Silently fail if kasa helper not available
        }
    }
...
print("mydesklight - macOS Keyboard Monitor")
print("=====================================")
...
if CommandLine.arguments.count > 1 {
    ...
    print("Usage: ./KeyboardMonitor <IP_ADDRESS>")
    exit(1)
}
